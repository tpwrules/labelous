...
# tokens. In fact, we use the edit key as a CSRF token because it's only
# available from a logged in user and SOP would prevent another page from
# requesting one. This also prevents us from having to try and smuggle the CSRF
# token through the tool. However, since the edit key is regenerated by a GET
# request, an "attacker" could use CSRF with e.g. an image to annoy the user by
# causing spurious inconsistency errors. We will accept this risk.
...
# the edit key behaves as a CSRF token so we use it instead of trying to smuggle
# Django's CSRF token through the tool.
@csrf_exempt
